...
#handMagicBtn{
  ...
  bottom:86px!important;z-index:88!important;min-height:56px!important;
  padding:0 22px!important;border-radius:999px!important;
  border:1px solid rgba(255,239,190,.98)!important;
  background:linear-gradient(180deg,#ffe39a,#d5a447)!important;color:#20150a!important;
  font-weight:900!important;font-size:14px!important;letter-spacing:.04em!important;white-space:nowrap!important;
  box-shadow:0 14px 38px rgba(0,0,0,.46),0 0 28px rgba(244,199,107,.42)!important;
...
@media(max-width:430px){
 .photo-chip{width:min(52vw,220px)!important;}.candle{width:50px!important;height:92px!important;}
 #viewer{padding-left:6px!important;padding-right:6px!important;}#viewer img{max-width:98vw!important;max-height:88vh!important;}
 #handMagicBtn{bottom:82px!important;min-height:54px!important;font-size:13px!important;padding:0 20px!important;}
 #handHud{width:112px;right:8px;bottom:146px;}#handVideo{width:112px;height:84px;}
}
`;
...
(function handMagic(){
 ...
 function waitForUi(){
  ...
  function nextCandle(){const c=[...document.querySelectorAll('.candle:not(.lit)')].find(el=>getComputedStyle(el).pointerEvents!=='none');if(c){c.click();setStatus('✨ Candle lit');mark();}else setStatus('All candles are lit ✨');}
  ...
  function nav(dir){const viewer=document.getElementById('viewer');if(viewer&&!viewer.classList.contains('hidden')){const b=document.getElementById(dir>0?'vNext':'vPrev');if(b)b.click();setStatus(dir>0?'👉 Next memory':'👈 Previous memory');mark();return;}if(dir>0)nextCandle();}
  const fingerUp=(lm,tip,pip)=>lm[tip].y<lm[pip].y-.035,dist=(a,b)=>Math.hypot(a.x-b.x,a.y-b.y);
  function onResults(res){
   if(!enabled)return;const lm=res.multiHandLandmarks&&res.multiHandLandmarks[0];
   if(!lm){setStatus('Show one hand');lastX=null;palmSince=0;return;}
   const now=Date.now(),x=1-lm[9].x,pinch=dist(lm[4],lm[8])<.055,open=fingerUp(lm,8,6)&&fingerUp(lm,12,10)&&fingerUp(lm,16,14)&&fingerUp(lm,20,18);
   if(pinch&&!pinchLatched&&cooldown(700)){pinchLatched=true;if(!openNewestMemory())setStatus('🤏 Pinch detected');}
   if(!pinch)pinchLatched=false;
   if(lastX!==null&&lastT&&cooldown(650)){const dx=x-lastX,dt=now-lastT;if(dt<450&&Math.abs(dx)>.16){nav(dx>0?1:-1);lastX=x;lastT=now;return;}}
   lastX=x;lastT=now;
   if(open){if(!palmSince)palmSince=now;if(now-palmSince>700&&cooldown(1100)){nextCandle();palmSince=now;}else if(now-palmSince<700)setStatus('✋ Hold palm to light');}else palmSince=0;
  }
  ...
  async function start(){
   try{btn.disabled=true;setStatus('Starting camera…');hud.classList.add('active');await load('https://cdn.jsdelivr.net/npm/@mediapipe/hands/hands.js');if(!window.Hands)throw new Error('Hand tracker failed to load');hands=new Hands({locateFile:f=>`https://cdn.jsdelivr.net/npm/@mediapipe/hands/${f}`});hands.setOptions({maxNumHands:1,modelComplexity:0,minDetectionConfidence:.6,minTrackingConfidence:.55});hands.onResults(onResults);stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:'user',width:{ideal:640},height:{ideal:480}},audio:false});video.srcObject=stream;await video.play();enabled=true;btn.classList.add('active');btn.textContent='✋ HAND MAGIC ON';btn.setAttribute('aria-pressed','true');setStatus('✋ Hold palm = candle\n🤏 Pinch = open\n↔ Swipe = navigate');frame();}
   catch(e){setStatus('Camera unavailable — tap still works');hud.classList.add('active');console.warn(e);}finally{btn.disabled=false;}
  }
  function stop(){enabled=false;cancelAnimationFrame(raf);if(stream)stream.getTracks().forEach(t=>t.stop());stream=null;video.srcObject=null;btn.classList.remove('active');btn.textContent='✋ HAND MAGIC';btn.setAttribute('aria-pressed','false');hud.classList.remove('active');setStatus('Camera off');}
