...
(function handMagic(){
 ...
 function waitForUi(){
  ...
  function nextCandle(){const c=[...document.querySelectorAll('.candle:not(.lit)')].find(el=>getComputedStyle(el).pointerEvents!=='none');if(c){c.click();setStatus('✨ Candle lit — photo stays until next candle');mark();}else setStatus('All candles are lit ✨');}
...
// Persistent candle-memory mode. The current candle's photo remains until the next candle is activated.
setTimeout(()=>{
  ...
  window.reveal=function(ids,candle){
    ...
    window.beauxMemoryNav=dir=>show(current+(dir>0?1:-1));
