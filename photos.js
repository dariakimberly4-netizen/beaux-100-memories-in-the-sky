...
// Persistent candle-memory swipe mode. This overrides the page's timed reveal after the inline script has loaded.
setTimeout(()=>{
  ...
  window.reveal=function(ids,candle){
    ...
    function show(index){
      ...
      hint.textContent=(ids.length>1?'← SWIPE • ':'')+'MEMORY '+(current+1)+' / '+ids.length+(ids.length>1?' • SWIPE →':'');
    }
